...
function AwardsTest() {
  ...
  return (
    <>
      ...
      <div className='container grid gap-6 p-5 mx-auto my-10 md:grid-cols-2 lg:grid-cols-4'>
        {awardsData.map((award, index) => (
          <div className='p-5 border rounded shadow' key={index}>
            <img src={award.img} alt={award.index} />
            <p className='pl-3 pr-3 mt-2 text-center text-gray-600'>
              {award.title}
            </p>
          </div>
        ))}
